...
export const useAccountsStorage = async ({socket, blockchain, create, sign}) => {
    ...
    const reconnectAddress = async (address) => {
        ...
        socket.emit('v2_addAddress', {address: account.address}, (response) => addAddressHandler({response, account}))
    }
    const reconnectAllAddresses = async () => {
        for (const account of accounts) {
            accountsStatus.set(account.address, 'AUTHORIZING')
            socket.emit('v2_addAddress', {address: account.address}, (response) => addAddressHandler({
                response,
                account
            }))
        }
    ...
    const addAddressHandler = async ({response, account}) => {
        const submitSignatureHandler = ({response, account}) => {
            if (response.success) {
                accountsStatus.set(account.address.toLowerCase(), 'ONLINE')
            } else {
                accountsStatus.set(account.address.toLowerCase(), 'UNAUTHORIZED')
                ElMessage.error('Что-то пошло не так: ' + account.address)
            }
        }

        if (response.success) {
            const salt = response.salt;
            const viemAcc = privateKeyToAccount(account.privateKey)
            const signature = await viemAcc.signMessage({
                message: {raw: salt}
            })
            socket.emit('v2_submitSignature', {
                address: account.address,
                salt,
                signature,
                blockchain,
                version
            }, (response) => submitSignatureHandler({response, account}))
        } else {
            accountsStatus.set(account.address.toLowerCase(), 'UNAUTHORIZED')
            ElMessage.error('Что-то пошло не так: ' + account.address)
        }
...
export const signTransactionHandler = (accountsStorage, socket) => async (message) => {
    const {addTransactionLog} = useTransactionsHistory()
    const from = message?.payload?.from
    const wallet = accountsStorage.getAccountByAddress(from)
    if (!wallet) return ElMessage.error('Адрес не найден: ' + from)

    const unsignedTx = message.payload.unsignedTx
    if (!unsignedTx) return ElMessage.error('Не указана транзакция для подписи. Адрес: ' + from)

    const parsedTx = parseTransaction(unsignedTx)
    const account = privateKeyToAccount(wallet.privateKey)
    try {
        const signature = await account.signTransaction(parsedTx)
        addTransactionLog({date: new Date(), txHash: message.payload.description, from})
...
export const signTransactionHandler_v2 = (accountsStorage) => async (message, callBack) => {
    const {addTransactionLog} = useTransactionsHistory()
    const from = message?.payload?.from
    const wallet = accountsStorage.getAccountByAddress(from)
    if (!wallet) {
        callBack({success: false, error: 'Адрес не найден: ' + from})
        return ElMessage.error('Адрес не найден: ' + from)
    }

    const {unsignedTx} = message.payload
    if (!unsignedTx) {
        callBack({success: false, error: 'Не указана транзакция для подписи. Адрес: ' + from})
        return ElMessage.error('Не указана транзакция для подписи. Адрес: ' + from)
    }
